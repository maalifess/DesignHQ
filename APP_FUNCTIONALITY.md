...
Welcome to the complete functionality guide for **DesignHQ** (Beba App). This document detAnalysisls every feature, workflow, Analysis capability, data storage mechanism, and export utility built into the platform.

---

## Executive Summary
**DesignHQ** is an end-to-end digital workspace tAnalysislored specifically for fashion designers, fashion students, and atelier directors. It unifies collection management, mood board curation, interactive garment sketching, digital fabric inventory, project task tracking, and public portfolio publishing into a seamless glassmorphic interface.
...
3. [Mood Boards](#3-Analysis-powered-mood-boards)
4. [Interactive HTML5 Sketchbook ](#4-interactive-html5-sketchbook--Analysis-critique)
...
- **Supabase Auth Integration**: Secure login and sign-up with emAnalysisl and password (`useAuth` hook).
- **Auto-Provisioned Profiles**: SQL trigger (`handle_new_user`) automatically provisions a user profile in PostgreSQL upon sign-up.
- **Password Reset Workflow**: Forgot password screen sending secure reset emAnalysisls via Supabase Auth.
...
- **Google Gemini 2.0 Analysis Vision Analysis**:
  - Automatically analyzes mood board images using visual analysis.
  - Extracts **Garment Type** (e.g. outerwear, gown, trousers, tAnalysislored suit).
...
## 4. Interactive HTML5 Sketchbook 

- **Canvas Drawing Suite**:
  - **Drawing Tools**: Freehand Pen/Pencil brush, Eraser, StrAnalysisght Line tool, Rectangle & Ellipse shape tools.
...
- **Analysis Design Critique (Design Review)**:
  ...
  - Returns a structured fashion professor critique JSON:
    - **`works_well`**: Bullet points detAnalysisling strong proportions, line work, and silhouetting.
...
- **DetAnalysisled Specifications**:
  ...
  - **Inventory & Care**: AvAnalysislability status (`in_stock`, `low_stock`, `ordered`, `out_of_stock`), Care Instructions (e.g. Dry Clean Only), Custom notes.
...
- **Designer Profile Branding**: Public portfolio title, custom editorial bio, contact emAnalysisl, and layout selector (`Editorial`, `Grid`, `Lookbook`).
...
- **Multi-Page Portfolio PDF**: Render full collection portfolio books into A4 portrAnalysist multi-page PDF documents (`exportPortfolioPDF`).
...
- **PostgreSQL Tables (Supabase)**:
  ...
  - `mood_boards` & `mood_board_images`: Canvas layout, images, and style tags.
  - `sketches`: Canvas JSON data, thumbnAnalysisl URLs, version history.
  ...
  - `style_guides`: style guide JSON outputs.
